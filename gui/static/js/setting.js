class FayInterface {
    ...
    handleIncomingMessage(data) {
        ...
        if (data.voiceList !== undefined) {
            ...
            if (typeof vueInstance.ensureVoiceSelected === 'function') {
                vueInstance.ensureVoiceSelected();
            }
        }
        if (data.robot) {
...
new Vue({
    ...
    methods: {
        initFayService() {
            ...
            const wsOrigin = 'ws://' + window.location.hostname + ':10003';
            this.fayService = new FayInterface(wsOrigin, apiOrigin, this);
        ...
        updateConfigFromData(config) {
            ...
            if (config.source && config.source.record) {
                ...
                if (!this.automatic_player_url || this.automatic_player_url.indexOf(':6000') !== -1) {
                    this.automatic_player_url = window.location.origin;
                };
        ...
        saveConfig() {
            let url = `${window.location.origin}/api/submit`;
            let send_data = {
                "config": {
                    "source": {
                        "liveRoom": {
        ...
        ensureVoiceSelected() {
            const vals = this.voiceList.map(v => v.value);
            if (this.voiceList.length > 0 && !vals.includes(this.attribute_voice)) {
                this.attribute_voice = this.voiceList[0].value;
            }
        },
